...
let $key = prompt(
  "Go to Openweathermap.org and sign up for a key. Once you have an API key, enter it below"
);
...
function start() {
  ...
  $.get(
    ...
    (data) => {
      ...
      const $weatherDiv = $(
        `<div id="weather" class="weather">${data.main.temp} °F</div>`
      );
      const $h1 = $(`<h3 class="card-title"></h3>`);
      const $img = $(
        `<img class="card-image" src="http://openweathermap.org/img/wn/${$icon}@2x.png"></ul>`
      );
      const $description = $(
